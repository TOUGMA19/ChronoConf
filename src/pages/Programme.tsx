...
import { Loader2, CalendarClock, Clock3, ShieldCheck, MapPin, Star, Flag, Award, Coffee, Mic, MoreHorizontal, Users2, UserCheck, ArrowRight } from "lucide-react";
      ...
      <header className="border-b border-border bg-card/60 backdrop-blur sticky top-0 z-10">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <Link to="/" className="flex items-center gap-2 font-display font-bold min-w-0">
            ...
            <span className="truncate">{data?.schedule.name || "Programme"}</span>
          </Link>
          {token && (
            <Button
              asChild
              size="sm"
              className="shrink-0 rounded-full gap-1.5 gradient-accent text-accent-foreground shadow-sm hover:shadow-md hover:brightness-105 transition-all px-3 sm:px-4 group"
            >
              <Link to={`/verify?t=${encodeURIComponent(token)}`}>
                <UserCheck className="h-3.5 w-3.5 shrink-0" />
                <span className="hidden sm:inline">Vérifier mes informations</span>
